...
-- Description: Creates spatial (GiST), unique partial, and performance indexes.
...
-- 4. Assignment Indexes & Partial Unique Index for Active Assignment
...
-- Enforces single active worker assignment rule per issue
...
-- 5. Status History Multi-Column Index
CREATE INDEX IF NOT EXISTS idx_status_history_issue_time ON issue_status_history(issue_id, created_at DESC);

-- 6. Notification Partial Index
CREATE INDEX IF NOT EXISTS idx_notifications_recipient_unread ON notifications(recipient_user_id) WHERE is_read = FALSE;
...
CREATE INDEX IF NOT EXISTS idx_ai_runs_model ON ai_prediction_runs(model_id);
CREATE INDEX IF NOT EXISTS idx_ai_runs_status ON ai_prediction_runs(run_status);

CREATE INDEX IF NOT EXISTS idx_ai_predictions_run ON ai_predictions(prediction_run_id);
CREATE INDEX IF NOT EXISTS idx_ai_predictions_category ON ai_predictions(category_id);
